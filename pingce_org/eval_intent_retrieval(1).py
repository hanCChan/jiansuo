...
def main() -> None:
    ...
    parser.add_argument(
        "--input",
        default=None,
        help="eval JSON path (relative to intent_eval/ root); default from config eval.input_json",
    )
    ...
    parser.add_argument(
        "--cache-dir",
        default=None,
        help="optional score cache dir (relative to intent_eval/ root); "
        "reuse cached per-query scores across runs",
    )
    parser.add_argument(
        "--no-cache",
        action="store_true",
    ...
    report = {
        ...
        "mode_doc": {
            ...
            "dense": {"dense": "SentenceTransformer dense only (Qwen etc.)"},
        },
        "models": {},
    }
    ...
    out_path = out_dir / eval_cfg.get("report_file", "intent_retrieval_eval.json")
